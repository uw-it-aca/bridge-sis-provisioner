...
class TestPurgeUserLoader(TransactionTestCase):
    ...
    def test_purge_user(self):
        with self.settings(RESTCLIENTS_PWS_DAO_CLASS=FPWS):
            ...
            self.assertEqual(d_loader.get_users_left_uw_count(), 2)
            self.assertEqual(d_loader.get_delete_count(), 0)
            ...
            user1 = BridgeUser.objects.get(netid='retiree')
            user1.save_terminate_date(graceful=False)
