...
class TestUserDao(TransactionTestCase):
    ...
    def test_netid_change(self):
        with self.settings(RESTCLIENTS_PWS_DAO_CLASS=FPWS):
            ...
            self.assertIsNotNone(person)
            bool_value, prev_netid = changed_netid([user], person)
            self.assertTrue(bool_value)
            self.assertEqual(prev_netid, 'renamed')
    ...
    def test_netid_regid_change(self):
        with self.settings(RESTCLIENTS_PWS_DAO_CLASS=FPWS):
            ...
            self.assertIsNotNone(person)
            bool_value, prev_netid = changed_netid([user1, user2], person)
            self.assertTrue(bool_value)
            self.assertEqual(prev_netid, 'retiree')
