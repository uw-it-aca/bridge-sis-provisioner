...
from sis_provisioner.csv.user_writer import make_import_user_csv_files,\
    ...
    make_netid_changed_user_csv_file, make_delete_user_csv_file
...
class CsvFileMaker:
    ...
    def make_load_user_files(self):
        if self.loader.get_loaded_count() == 0:
            return 0
        try:
            number_users_wrote = make_import_user_csv_files(
                self.loader.worker.get_users_to_load(),
                self.filepath,
    ...
    def make_netid_change_user_file(self):
        if self.loader.get_netid_changed_count() == 0:
            return 0
        try:
            number_users_wrote = make_netid_changed_user_csv_file(
                self.loader.worker.get_users_netid_changed(),
                self.filepath,
                self.include_hrp_data)
            self._set_file_wrote()
    ...
    def make_regid_change_user_file(self):
        if self.loader.get_regid_changed_count() == 0:
            return 0
        try:
            number_users_wrote = make_regid_changed_user_csv_file(
                self.loader.worker.get_users_regid_changed(),
                self.filepath,
                self.include_hrp_data)
            self._set_file_wrote()
    ...
    def make_delete_user_file(self):
        if self.loader.get_deleted_count() == 0:
            return 0
        try:
            number_users_wrote = make_delete_user_csv_file(
                self.loader.worker.get_users_to_delete(),
                self.filepath)
            self._set_file_wrote()
    ...
    def make_restore_user_file(self):
        if self.loader.get_restored_count() == 0:
            return 0
        try:
            number_users_wrote = make_restore_user_csv_file(
                self.loader.worker.get_users_to_restore(),
                self.filepath,
                self.include_hrp_data)
            self._set_file_wrote()
            logger.info("Total %d users (to restore) wrote into %s.\n" % (
                    number_users_wrote, self.filepath))
            return number_users_wrote
        except Exception:
            log_exception(logger,
                          "Failed to make restore user file in %s" %
                          self.filepath,
                          traceback.format_exc())
            raise
